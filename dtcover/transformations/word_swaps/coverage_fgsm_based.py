...
from dtcover.cover_target import get_cover_grad
...
from dtcover.metrics import CovKind, CovType, NeuralMetric

from dtcover.metrics import CovType
from dtcover.metrics.ncov_pos_atten import NCovPosAtten
from dtcover.metrics.ncov_mlp import NCovMlp
from dtcover.metrics.kmncov_mlp import KMNCovMlp
from dtcover.metrics.kmncov_pos_atten import KMNCovPosAtten
from dtcover.metrics.nbcov_mlp import NBCovMlp
from dtcover.metrics.nbcov_pos_atten import NBCovPosAtten
from dtcover.metrics.snacov_mlp import SNACovMlp
from dtcover.metrics.snacov_pos_atten import SNACovPosAtten


class WordSwapCoverageFGSMBased(WordSwap):
    def __init__(self, model_wrapper, samplek=1, alpha=0.5, scale_max=3, scale_step=0.3, cov_kind=CovKind.Mlp, logger=None):
    ...
    def _get_replacement_words_by_grad(self, attacked_text: AttackedText, indices_to_replace: List[int]):
        ...
        for j, word_idx in enumerate(indices_to_replace):
            matched_tokens = word2token_mapping[word_idx]
            if matched_tokens is None:
                continue
            candidates = self._gen_adv(text_ids, word_idx, matched_tokens, emb_grad, lookup_table)
