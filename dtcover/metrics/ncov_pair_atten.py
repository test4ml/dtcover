...
import torch
from dtcover.coverage.coverage_utils import scale_batch, scale_batch_clean, scale_batch_dim
from dtcover.coverage.neuron_coverage import NeuronCoverage
from dtcover.metrics import CovKind, NeuralMetric

class NCovPairAtten(NeuralMetric):
    ...
    def update(self, layer_name: str, layer: Union[torch.Tensor, Tuple[torch.Tensor]]):
        ...
        if layer_name not in self.state:
            self.state[layer_name] = torch.zeros(self.max_seq_len, self.max_seq_len, dtype=torch.bool, device=self.device)
        ...
        diff_sum = torch.any(torch.any(diff, 1), 0).to(self.device)
        # [batch, num_heads, seq, seq] -> [num_heads, seq]
        self.state[layer_name][:sequence_length,:sequence_length] |= diff_sum
    ...
    def compute(self):
        ...
        for l, v in self.state.items():
            ...
            total += (self.max_seq_len + 1) * self.max_seq_len // 2
        return cover / total
